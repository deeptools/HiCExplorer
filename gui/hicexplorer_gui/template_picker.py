...
class TemplatePicker(QtWidgets.QDialog):
    ...
    def __init__(self, entries, parent=None, templates=None):
        ...
        self.unavailable.setAlignment(QtCore.Qt.AlignTop | QtCore.Qt.AlignLeft)
        self.unavailable.setObjectName("unavailable_steps")
        unavailable_scroll = QtWidgets.QScrollArea()
        unavailable_scroll.setWidgetResizable(True)
        unavailable_scroll.setHorizontalScrollBarPolicy(QtCore.Qt.ScrollBarAlwaysOff)
        unavailable_scroll.setWidget(self.unavailable)
        unavailable_scroll.setMinimumHeight(60)
        # The parameters and the unavailable steps share the height; both
        # scroll vertically, so neither is cut off on a small screen.
        splitter = QtWidgets.QSplitter(QtCore.Qt.Vertical)
        splitter.setChildrenCollapsible(False)
        splitter.addWidget(scroll)
        splitter.addWidget(unavailable_scroll)
    ...
    def _show(self, _row):
        ...
        for parameter in template.parameters:
            ...
            if parameter["kind"] in ("file", "files", "directory"):
                ...
                browse.setText("...")
                browse.setToolTip("Choose {}".format(parameter["name"]))
