...
#  3. HICX_PLOT_ALLOW_UNPINNED=1 accepts an older version with a warning.
...
wrong=$(stub wrong matplotlib '__version__ = "3.7.0"')
...
expect "the message names both versions" \
    grep -q "matplotlib 3.7.0 is installed, but matplotlib 3.8 or newer is required" err.txt
...
expect "the message says it is not installed" \
    grep -q "matplotlib is not installed .*(required: matplotlib 3.8 or newer)" err.txt
...
expect "and warns" grep -q "warning: hicInterIntraTAD draws with matplotlib 3.7.0" err.txt
...
unchanged_after_refusal() {
    ...
    expect "$name: the refusal names the versions" \
        grep -q "matplotlib 3.7.0 is installed, but matplotlib 3.8 or newer is required" \
