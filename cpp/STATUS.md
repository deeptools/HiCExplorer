...
|---|---|---|---|
| `hicCorrectMatrix --correctionMethod KR` | float32 downcast of input values (`krbalancing.cpp:27`); float32 rescale accumulators (`:228-229`), but summed in a fixed order so the C++ is deterministic where the Python is not; the `--perchr` `.h5`-vs-`.cool` correction-factor discrepancy (finding F4) | float64 throughout; pairwise accumulation; `--perchr` rescales consistently | not yet measured; expected to be of the order of the oracle's own noise envelope, 1e-4 to 1e-2 relative depending on matrix size |
...
Defects found while planning, worth reporting upstream. F1 and F4 change what
the port can promise; F5 is a silent-success failure mode.
...
| **F4** | `hicCorrectMatrix.py:715-732` | On the `--perchr` KR path, `rescale_norm_vector()` is only triggered by `get_normalised_matrix(True)`, which is only called when the output name ends in `.h5` (`:726`). The following `get_normalisation_vector(False)` (`:731`) therefore returns **rescaled** factors for `.h5` output and **unrescaled** factors for `.cool` output. The same command with a different output extension gives different correction factors. The Python suite never runs `--perchr`. |
