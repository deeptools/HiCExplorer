...
resolution matrices (larger bins) can be easily constructed using [hicMergeMatrixBins](tools/hicMergeMatrixBins.md). Matrices
at restriction fragment resolution can be created by providing a file
containing the restriction sites, this file can be created with the tool [hicFindRestSite](tools/hicFindRestSite.md)

[hicFindRestSite](tools/hicFindRestSite.md)
...
enriched by reads should be discarded. To aid in the filtering of regions [hicCorrectMatrix](tools/hicCorrectMatrix.md) generates a
...
![Histogram of the number of counts per bin.](images/diagnostic_plot.png)
...
```
![Corrected Hi-C counts in log scale.](images/corrected_matrix_example.png)
...
way [Lieberman-Aiden ](http://doi.org/10.1126/science.1181369) describes it. In a next step a pearson 
...
The A / B compartments can be plotted with [hicPlotMatrix](tools/hicPlotMatrix.md).

```bash
$ hicPlotMatrix -m pearson_all.h5 --outFileName pca1.png --perChr --bigwig pca1.bw
```
![](images/eigenvector1_lieberman.png)
