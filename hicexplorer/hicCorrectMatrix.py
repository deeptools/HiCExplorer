...
"""
    ...
    merge_mode.add_argument('--filterThreshold',
                            '-t',
...
def merge_failed_bins(hic_matrix, failed_bins):
    ...
    def get_merged_bin(bin_list):
        ...
        assert len(
            bin_list) > 1, "Error, bin_list length has less than 2 elements."
            ...
            if idx > 1 and ref_name_list[idx - 1] != ref_name_list[idx]:
                if consecutive:
                    if len(consecutive) == 1:
                        new_bins.append(
                            hic_matrix.cut_intervals[consecutive[0]])
...
def main():
    ...
    if 'outMatrixFile' in args:
        # get below threshold outliers by using an extremely high upper
        # threshold
        outlier_regions = filter_by_zscore(
            ma, args.filterThreshold[0], 1e6, perchr=args.perchr)
