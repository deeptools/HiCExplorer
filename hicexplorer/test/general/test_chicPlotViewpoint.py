...
def test_two_viewpoint_fileformat_dpi():
    ...
    res = compare_images(ROOT + 'chicPlotViewpoint/two_viewpoint.pdf', outfile.name, tolerance)
    assert res is None, res
...
def test_one_viewpoint_per_file_batch_mode():
    ...
    args = "-if {} -iff {} --range {} {} --outputFolder {} -psn {} -bm".format(ROOT + 'chicViewpoint/fileNames_one_matrix.txt',
                                                                               ROOT + 'chicViewpoint/output_4',
                                                                               200000, 200000,
                                                                               output_folder, 1).split()
    chicPlotViewpoint.main(args)

    res = compare_images(ROOT + 'chicPlotViewpoint/batchMode/one/Eya1_FL-E13-5_chr1_chr1_14300280.png', output_folder + '/Eya1_FL-E13-5_chr1_chr1_14300280.png', tolerance)
    assert res is None, res
    res = compare_images(ROOT + 'chicPlotViewpoint/batchMode/one/Sox17_FL-E13-5_chr1_chr1_4487435.png', output_folder + '/Sox17_FL-E13-5_chr1_chr1_4487435.png', tolerance)
    assert res is None, res
