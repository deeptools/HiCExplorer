            ...
            for file in pFileList:
                ...
                for sample in file:
                    ...
                    if pArgs.outputFileType == 'txt':
                        ...
                        for key in key_list:
                            file_content_string += '\t'.join('{:.{decimal_places}f}'.format(x, decimal_places=pDecimalPlace) if isinstance(x, np.float64) else str(x) for x in data[1][key]) + '\n'
                    else:
                        for key in key_list:
            ...
            for file in pFileList:
                for sample in file:
                    ...
                    for line in line_content:
                        file_content_string += '\t'.join('{:.{decimal_places}f}'.format(x, decimal_places=pDecimalPlace) if isinstance(x, np.float64) else str(x) for x in line) + '\n'
                    file_content_list.append(file_content_string)
