...
def parse_arguments(args=None):
    ...
    parserOpt.add_argument('--avgType',
                           help='Type of average to compute final matrix. Options are mean, median and sum. Default is mean.',
                           choices=['mean', 'median', 'sum'],
                           default='mean')

    parserOpt.add_argument('--genome',
                           help='If given, submatrices are average over all the genome.',
                           action='store_true',
                           required=False)

    parserOpt.add_argument('--regionReferencePosotion',
                           help='wether centering the window around the left, the center or the right of the regions.',
                ...
                for idx, chrom in enumerate(chrom_matrix):
                    for cluster_number, cluster_indices in enumerate(cluster_ids[chrom]):
                        ...
                        if num_clusters == 1:
                            ...
                            output_matrix_name = "{file}_{chrom}_cluster_{id}.tab".format(file=args.outFilePrefixMatrix, chrom=chrom, id=cluster_number + 1)
                        np.savetxt(output_matrix_name, chrom_avg[chrom][cluster_number], '%0.5f', delimiter='\t')
...
def main(args=None):
    ...
    #ma=hm.hiCMatrix("/data/users/scollomb/Lab_Heard/project_Katia_scHiC/HiC/HicPro/scHiC_data_embryo/EHP_182/cooler/EHP_182_G1.10kb.cool")
    #regionsFile=open("/data/users/scollomb/Lab_Heard/project_Katia_scHiC/HiC/3dnetmod/TADs_quant_pseudoBulk/3dnetmod.mergeTADs.cellThetaCc1_all_G1G2/Cmean/AllCluster/bed_superClusters_l01_center/1_l01.bed", "r")
    ...
    for chrom in chrom_list:
        ...
        for start, end in bed_intervals[chrom]:
            ...
            if args.regionReferencePosotion=="left":
                bin_id = ma.getRegionBinRange(toString(chrom), start, start+1)
            elif args.regionReferencePosotion=="right":
                bin_id = ma.getRegionBinRange(toString(chrom), end-1, end)
            elif args.regionReferencePosotion=="center":
    ...
    if args.genome :
        log.info("Making plots for genom-wide average")
        plot_aggregated_contacts(genome_matrix, genome_chrom_contact_position, genome_cluster_ids, num_clusters, M_half, args)
        ...
        if args.outFileContactPairs:
            ...
            for genome_cluster_number, genome_cluster_indices in enumerate(genome_cluster_ids["genome"]):
