...
def parse_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(
        ...
        add_help=False,
        description="""
chicQualityControlBackground computes a quality control for the background model over all given samples with all reference points. For all relative distances to a reference point
...
$ chicQualityControlBackground --matrices matrix1.cool matrix2.cool matrix3.cool --referencePoints referencePointsFile.bed --range 20000 40000 --outFileName background_model.bed
"""
    )
...
def main(args=None):
    ...
    for relative_position in relative_positions:
        ...
        all_data.append(background_model_data[relative_position])
    
    relative_positions = np.array(relative_positions)
    all_data = np.array(all_data)
    np.save('relative_positions.npy', relative_positions)
