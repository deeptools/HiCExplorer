...
""")
    ...
    find_tads_subparser = subparsers.add_parser(
        'find_TADs', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def find_consensus_minima(tad_score_matrix, lookahead=3, chrom=None):
    ...
    _max, _min = peakdetect(tad_score_matrix_avg,
                            lookahead=lookahead, chrom=chrom)
...
def save_domains_and_boundaries(chrom, chr_start, chr_end, matrix, min_idx,
    ...
    # the indices at which the name of the chromosome changes (chrom, start,
    # end should be  sorted)
    unique_chroms, chr_start_idx = np.unique(chrom, return_index=True)
...
def compute_spectra_matrix(args, matrix=None):
    ...
    incremental_step = get_incremental_step_size(
        args.minDepth, args.maxDepth, args.step)
    ...
    hic_ma.matrix = sparse.triu(
        hic_ma.matrix, k=0, format='csr') - sparse.triu(hic_ma.matrix, k=limit, format='csr')
...
def min_pvalue(matrix_file, min_idx, chrom, chr_start, chr_end, window_len=20000, delta=None):
    ...
    for idx in min_idx:
        ...
        matrix_idx = hic_ma.getRegionBinRange(
        ...
        if len(left) == 0 and len(right) == 0:
            ...
            try:
                ...
                # this condition happens when boundary, left and right are only
                # composed of 'zeros'
                pval = np.nan
...
def min_pvalue_diamond_vs_triangle(matrix_file, min_idx, chrom, chr_start, chr_end, window_len=20000, delta=None):
    ...
    for idx in min_idx:
        ...
        if triangle is None or len(triangle) == 0:
            ...
            try:
                pval = ranksums(
                    boundary[boundary.nonzero()], triangle[triangle.nonzero()])[1]
            except ValueError:
                # this condition happens when boundary, left and right are only
                # composed of 'zeros'
                pval = np.nan
...
def main(args=None):
    ...
    if args.command == 'TAD_score':
        ...
        save_bedgraph_matrix(args.outFileName, chrom,
                             chr_start, chr_end, matrix, args)
        return

    chrom, chr_start, chr_end, matrix, parameters = load_spectrum_matrix(
        args.tadScoreFile)
    ...
    min_idx, delta = find_consensus_minima(
        matrix, lookahead=lookahead, chrom=chrom)
    ...
    save_domains_and_boundaries(
        chrom, chr_start, chr_end, matrix, min_idx, delta, pvalues, args)

    # turn of hierarchical clustering which is apparently not working.
    # if 2 == 1:
    #     boundary_list = [(hic_ma.cut_intervals[min_][0],
    #                       hic_ma.cut_intervals[min_][2], mean_mat[min_]) for min_ in min_idx]

    #     Z, clusters = hierarchical_clustering(
    #         boundary_list, clusters_cutoff=[0.4, 0.3, 0.2])
